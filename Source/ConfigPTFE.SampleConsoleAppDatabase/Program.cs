...
namespace ConfigPTFE.SampleConsoleAppDatabase {
	internal class Program {
		private static void Main(string[] args) {
			...
			Console.WriteLine(
				"\r\n\r\nUsing the named connectionString 'Rivendell' to get the appSetting values located in the database Rivendell{0}",
		...
		private static void BootstrapConfigPTFE(string environmentnameChoice) {
			...
			ConfigPTFE.AdvancedConfigurationProvider.Initialize(
				x => {
					...
					x.ConfigSectionLocations.Add("COMMONSETTINGS", @"C:\SourceControl\Configuration\CommonSettings");
					x.ConfigSectionLocations.Add("CONNECTIONSTRINGS", @"C:\SourceControl\Configuration\Connectionstrings");
					x.AesKey = GetAesKey();
					x.AesIV = GetAesIV();
				}
				);
